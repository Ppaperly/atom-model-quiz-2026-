...
document.querySelector("#adminOpenButton").addEventListener("click", renderAdminLogin);
init();
...
function renderStart() {
  views.start.innerHTML = `<section class="panel stack"><div><h2>학생 정보 입력</h2><p class="muted">반, 조, 번호, 이름을 입력한 뒤 시작하세요.</p></div><form id="startForm" class="stack"><div class="form-grid"><label>반 <input name="className" required placeholder="예: 1반" /></label><label>조 <input name="teamName" required placeholder="예: 3조" /></label><label>번호 <input name="studentNumber" inputmode="numeric" required placeholder="예: 12" /></label><label>이름 <input name="studentName" required placeholder="예: 김민준" /></label></div><div class="actions"><button type="submit">게임 시작</button>${state.settings.elementSearchEnabled ? `<button id="openSearchButton" class="secondary-button" type="button">원소 검색</button>` : ""}</div></form></section>`;
...
async function handleStart(event) {
  ...
  const profile = { className: clean(form.get("className")), teamName: clean(form.get("teamName")), studentNumber: clean(form.get("studentNumber")), studentName: clean(form.get("studentName")) };
...
async function handleFeedbackSubmit(event) {
  ...
  if (!message) return;
  event.submitter.disabled = true;
  try { await saveFeedback({ submissionId: event.currentTarget.dataset.feedbackForm, fromClassName: state.profile.className, fromTeamName: state.profile.teamName, fromStudentNumber: state.profile.studentNumber, fromStudentName: state.profile.studentName, message }); showMessage("피드백을 등록했습니다."); await renderResults("all"); }
...
function renderAdminSubmissionList() { const keyword = clean(views.admin.querySelector("#adminFilter")?.value).toLowerCase(); const submissions = state.submissions.filter((item) => !keyword || `${item.className} ${item.teamName} ${item.studentNumber} ${item.studentName}`.toLowerCase().includes(keyword)); views.admin.querySelector("#adminSubmissionList").innerHTML = submissions.length ? submissions.map((item) => renderSubmissionCard(item, true)).join("") : `<p class="muted">조건에 맞는 제출물이 없습니다.</p>`; }
